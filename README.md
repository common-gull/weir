...
`src/tools/` ships generic primitives (`gh`, `ghGraphql`, `git`, `runClaude`, …). To add your own
reusable tools/helpers or share code between workflows, put modules in [`lib/`](lib/README.md) and
import them — no engine edits. Outward actions are gated by capabilities; declare a custom one with
