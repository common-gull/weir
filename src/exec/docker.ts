// Rung-2 image pinning (#C8): resolve a docker image tag/ref to its content digest so a step's
...
// function (unit-tested); resolution shells out to `docker image inspect`, gated in tests behind a
// runtime-availability check so `bun run check` stays green without Docker.
...
/** Extract the canonical `sha256:<hex>` content digest from `docker image inspect`'s RepoDigests
...
export function parseRepoDigest(output: string): string {
    ...
    if (!m) throw new Error(`no sha256 repo digest in docker inspect output: ${JSON.stringify(output)}`);
    return m[0];
}
