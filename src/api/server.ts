...
export function createServer(deps: ServerDeps) {
  ...
  const handlers: Array<{ m: string; re: RegExp; fn: (req: Request, m: RegExpMatchArray, url: URL) => Response | Promise<Response> }> = [];
  const on = (m: string, re: RegExp, fn: (req: Request, mm: RegExpMatchArray, url: URL) => Response | Promise<Response>) =>
    handlers.push({ m, re, fn });

  on('GET', /^\/api\/workflows$/, () => {
    const rows = allWorkflows().map((wf) => {
      const last = db
        .query(`SELECT id, status, created_at, finished_at FROM runs WHERE workflow = ? ORDER BY created_at DESC LIMIT 1`)
        .get(wf.name);
      const counts = db
  ...
  on('GET', /^\/api\/runs$/, (_req, _m, url) => {
    ...
    if (wf) (where.push('workflow = ?'), args.push(wf));
  ...
  on('GET', /^\/api\/runs\/([^/]+)\/events$/, (_req, m, url) => {
    ...
    const rows = (
      before
        ? db.query(`SELECT * FROM events WHERE run_id = ? AND id < ? ORDER BY id DESC LIMIT ?`).all(id, before, limit)
  ...
  on('GET', /^\/api\/stream$/, (req, _m, url) => {
    ...
    if (!cursor) {
      const last = db.query(`SELECT MAX(id) AS m FROM events${runId ? ' WHERE run_id = ?' : ''}`).get(
        ...(runId ? [runId] : []),
      ) as { m: number | null };
      cursor = last?.m ?? 0;
    }
    ...
    const stream = new ReadableStream({
      start(controller) {
        ...
        const drain = () => {
          const rows = (
            runId
              ? db.query(`SELECT * FROM events WHERE id > ? AND run_id = ? ORDER BY id LIMIT 500`).all(cursor, runId)
