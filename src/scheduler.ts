...
export class Scheduler {
  ...
  constructor(private db: DB, private onFire?: () => void, private now: () => number = Date.now) {}
  ...
  syncFromRegistry(): { upserted: string[]; removed: string[] } {
    ...
    for (const wf of allWorkflows()) {
      ...
      if (existing) {
        this.db
          .query(`UPDATE schedules SET workflow = ?, cron = ?, tz = ?, input = ?, overlap = ?, catchup = ?, backfill_max = ? WHERE id = ?`)
          .run(wf.name, s.cron, tz, toJson(s.input), s.overlap ?? 'skip', s.catchup ?? 'skip', s.backfillMax ?? 24, id);
      } else {
        this.db
          .query(
  ...
  private overlapAllows(s: ScheduleRow): boolean {
    ...
    if (s.overlap === 'cancel-previous') {
      for (const a of active)
        this.db.query(`UPDATE runs SET status = 'cancelled', finished_at = ? WHERE id = ?`).run(Date.now(), a.id);
      return true;
    }
    return false; // skip
  }
