...
export function defineWorkflow(name: string, a: any, b?: any): WorkflowDef {
...
function computeBackoff(attempt: number, b: NonNullable<StepOpts['retries']>['backoff']): number {
  ...
  let d = type === 'fixed' ? base : base * Math.pow(b?.factor ?? 2, attempt);
...
function buildCtx(
  ...
  async function runStepBody<T>(
    ...
    for (;;) {
      ...
      iters++;
      const cont =
        (opts.repeat.until && !opts.repeat.until(result)) ||
        (opts.repeat.while && opts.repeat.while(result));
  ...
  const stepImpl = <T>(name: string, fn: (s: StepCtx) => T | Promise<T>, opts: StepOpts<T> = {}, keyOverride?: string) => {
  ...
  const ctx: Ctx = {
    ...
    loop: async <T>(opts: LoopOpts<T>, bodyFn: (it: LoopCtx) => T | Promise<T>): Promise<T> => {
      ...
      for (let i = 0; i < opts.max; i++) {
        ...
        prev = result;
        const done =
          (opts.until && opts.until(result)) || (opts.while && !opts.while(result));
        if (done) break;
      }
      return result;
    },
...
function syncMemo(db: DB, runId: string, state: Replay, kind: StepKind, name: string, produce: () => any): any {
  const seq = state.seq++;
  const hit = state.memoBySeq.get(seq);
  if (hit) return fromJson(hit.result);
  const value = produce();
...
function kvGetRaw(db: DB, ns: string, key: string): string | null | undefined {
  const row = db.query(`SELECT value, expires_at FROM kv WHERE namespace = ? AND key = ?`).get(ns, key) as
    | { value: string | null; expires_at: number | null }
    | null;
