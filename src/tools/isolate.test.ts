...
test('a throw inside custom JS becomes a clean step error', async () => {
  await expect(runIsolated('export default () => { throw new Error("nope") }', null, { timeoutMs: 10_000 })).rejects.toThrow(
    /isolated step failed: nope/,
  );
});

test('non-function default is rejected clearly', async () => {
  await expect(runIsolated('export default 42', null, { timeoutMs: 10_000 })).rejects.toThrow(/must .export default. a function/);
});
