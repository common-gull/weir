...
const stepNames = (runId: string) =>
  (db.query(`SELECT key FROM steps WHERE run_id = ? ORDER BY seq`).all(runId) as { key: string }[]).map(
    (r) => r.key,
  );
...
test('retry-from-failure: completed steps do not re-run; execution resumes at the failure', async () => {
  ...
  defineWorkflow('t', {}, async (ctx) => {
    await ctx.step('a', () => { ran.push('a'); return 1; });
    await ctx.step('b', () => { ran.push('b'); return 2; });
    await ctx.step('c', () => {
      ran.push('c');
      if (failReview) throw new Error('boom');
      return 3;
    });
    await ctx.step('d', () => { ran.push('d'); return 4; });
    return 'ok';
  });
...
test('ctx.loop: bounded auto-iteration, fails twice then succeeds -> exactly 3 iterations', async () => {
  let attempts = 0;
  defineWorkflow('t', {}, async (ctx) => {
    const out = await ctx.loop({ max: 5, until: (r: any) => r.ok }, async (it) => {
      const ok = (await it.step('try', () => { attempts++; return attempts >= 3; })) as boolean;
      return { ok };
    });
    return out;
  });
...
test('retry --from: discards memo at/after a step so it re-runs', async () => {
  ...
  defineWorkflow('t', {}, async (ctx) => {
    await ctx.step('a', () => { ran.push('a'); return 1; });
    await ctx.step('b', () => { ran.push('b'); return 2; });
    await ctx.step('c', () => { ran.push('c'); return 3; });
    return 'ok';
  });
  const id = createRun(db, 't');
  await executeRun(db, id);
...
test('retryRun --from matches step names exactly (no substring over-delete)', async () => {
  const ran: string[] = [];
  defineWorkflow('t', {}, async (ctx) => {
    await ctx.step('redeploy-check', () => { ran.push('redeploy-check'); return 1; });
    await ctx.step('deploy', () => { ran.push('deploy'); return 2; });
    return 'ok';
  });
  const id = createRun(db, 't');
  await executeRun(db, id);
