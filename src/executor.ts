...
export class Executor {
    ...
    sweepOrphans(): number {
        ...
        const res = this.db
            .query(`UPDATE runs SET status = 'interrupted', finished_at = ? WHERE status = 'running'`)
            .run(Date.now());
    ...
    cancel(runId: string): void {
        ...
        this.db
            .query(`UPDATE runs SET status = 'cancelled', finished_at = ? WHERE id = ? AND status = 'queued'`)
            .run(Date.now(), runId);
    }
}
