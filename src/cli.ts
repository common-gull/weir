...
async function cmdStart(cfg: WeirConfig): Promise<void> {
  ...
  for (const u of unknownCapabilities(allWorkflows())) {
    console.warn(`⚠ workflow "${u.workflow}" declares undeclared capability "${u.capability}" — still enforced; defineCapability() it to document (see lib/README.md)`);
  }

  const executor = makeExecutor(db, cfg);
  const scheduler = new Scheduler(db, () => executor.wake());
  const server = createServer({ db, executor, scheduler, workflowsDir: cfg.workflowsDir, uiDir: cfg.uiDir, port: cfg.port });
...
async function cmdReload(cfg: WeirConfig): Promise<void> {
  ...
  const body = (await res.json().catch(() => ({}))) as { workflows?: number; files?: number; removed?: string[]; error?: string };
  if (!res.ok) {
